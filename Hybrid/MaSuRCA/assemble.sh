...
CONFIG_PATH="/labs/Wegrzyn/genome_assembly_tutorial/masurca/config.txt"
CMD_PATH="/isg/shared/apps/MaSuRCA/3.3.4/bin/masurca"
set -o pipefail
...
#}
export PATH="/isg/shared/apps/MaSuRCA/3.3.4/bin/../CA8/Linux-amd64/bin:/isg/shared/apps/MaSuRCA/3.3.4/bin:$PATH"
save PATH
export PERL5LIB=/isg/shared/apps/MaSuRCA/3.3.4/bin/../lib/perl${PERL5LIB:+:$PERL5LIB}
save PERL5LIB
NUM_THREADS=16
save NUM_THREADS
log 'Processing pe library reads'
rm -rf meanAndStdevByPrefix.pe.txt
echo 'aa 300 50' >> meanAndStdevByPrefix.pe.txt

head -q -n 40000  aa.renamed.fastq | grep --text -v '^+' | grep --text -v '^@' > pe_data.tmp
...
CA_DIR="CA";
/isg/shared/apps/MaSuRCA/3.3.4/bin/mega_reads_assemble_cluster.sh  -E SLURM  -Pb 500000000 -q general -G 1 -C 25 -t 16 -e $ESTIMATED_GENOME_SIZE -m work1 -a /isg/shared/apps/MaSuRCA/3.3.4/bin/../CA8/Linux-amd64/bin -o "   cgwErrorRate=0.15"  -B 15 -D 0.02 -n /labs/Wegrzyn/Moss/Physcomitrellopsis_africana/Physcomitrellopsis_africana_Genome/RawData_Nanopore_5074/5074_test_LSK109_30JAN19/5074_test_LSK109_30JAN19-reads/5074_test_LSK109_30JAN19-reads-pass.fastq 
...
if [ -s $CA_DIR/9-terminator/genome.scf.fasta ];then
  ...
  if [ $NCTG -eq $NSCF ];then
    ...
    if [ -s $CA_DIR/10-gapclose/genome.scf.fasta ];then
      ...
      log 'Gap closing'
      closeGapsLocally.perl --max-reads-in-memory 1000000000 -s 10000000000 --Celera-terminator-directory $CA_DIR/9-terminator --reads-file 'aa.renamed.fastq' --output-directory $CA_DIR/10-gapclose --min-kmer-len 17 --max-kmer-len $(($PE_AVG_READ_LENGTH-5)) --num-threads 16 --contig-length-for-joining $(($PE_AVG_READ_LENGTH-1)) --contig-length-for-fishing 200 --reduce-read-set-kmer-size 21 1>gapClose.err 2>&1
